...
#+begin_src emacs-lisp
  ...
  (use-package dashboard
    ...
    (setq dashboard-items '((recents . 5)
                            (bookmarks . 3)
                            (projects . 3)))  ;; Removed agenda to speed up
    :config
    ...
    (use-package general
      ...
      (my/general-define-key
       (("M-w" 'easy-kill
         ...
         "C-c j y" 'my/bmkp-copy-file-path
        ...
        (bookmark-bmenu-mode-map
           "C-c y" 'my/bmkp-copy-file-path-at-point)
...
#+begin_src emacs-lisp
  (use-package bookmark+
    ...
    :defer t
    :commands (bmkp-jump-to-type bmkp-set-bookmark-file-bookmark))

  (defun my/bmkp-copy-file-path (bookmark)
  "Copy the file path of BOOKMARK to the kill ring."
  (interactive (list (bookmark-completing-read "Bookmark" (bmkp-default-bookmark-name))))
  (let ((file (bookmark-get-filename bookmark)))
    (when file
      (kill-new file)
      (message "Copied to kill ring: %s" file))))

  (defun my/bmkp-copy-file-path-at-point ()
  "Copy the file path of bookmark at point to the kill ring."
  (interactive)
  (let* ((bmk (bookmark-bmenu-bookmark))
    ...
    (if file
        (progn
          (kill-new file)
          (message "Copied to kill ring: %s" file))
      (message "No file associated with this bookmark"))))
...
#+begin_src emacs-lisp
  ...
  (use-package casual-lib
      :elpaca (casual-lib :host github :repo "kickingvegas/casual-lib")
      :after (re-builder))  

    (use-package casual-re-builder
      :elpaca (casual-re-builder :host github :repo "kickingvegas/casual-re-builder")
      :after (re-builder))
    ...
    (use-package casual-info
      :elpaca (casual-info :host github :repo "kickingvegas/casual-info"))

    (use-package casual-bookmarks
      :elpaca (casual-bookmarks :host github :repo "kickingvegas/casual-bookmarks"))
#+end_src
* Editing
** Avy
#+begin_src emacs-lisp
...
# ** Claude Code TRAMP Integration

# #+begin_src emacs-lisp
#     ;; Fix for vterm backend over TRAMP
#     (with-eval-after-load 'vterm
#       (setq vterm-tramp-shells `((t ,(executable-find "claude")))))
    
#     ;; SSH port forwarding for MCP server connection
#     (with-eval-after-load 'claude-code-ide
#       (defun claude-code-ide-start-ssh-forwarding (port host)
#         "Start SSH port forwarding from remote host to local port for MCP server."
#         (when (and port host)
#           (claude-code-ide-debug "Starting SSH Forwarding MCP HTTP for %d" port)
#           (call-process "ssh" nil nil nil "-O" "forward" "-R"
#                         (format "%d:localhost:%d" port port)
#                         host)))
      
#       ;; Hook into the MCP server setup to enable port forwarding for remote files
#       (advice-add 'claude-code-ide-ensure-http-server :before
#                   (lambda ()
#                     (when (file-remote-p default-directory)
#                       (let ((actual-port (or claude-code-ide-http-port 
#                                            (claude-code-ide-get-free-port))))
#                         (claude-code-ide-start-ssh-forwarding 
#                          actual-port
#                          (file-remote-p default-directory 'host)))))))
# #+end_src

# *** Remote Claude Wrapper Script

# For the PID lockfile issue, create this wrapper script on your remote host and ensure it's in your PATH as 'claude':

# #+begin_example 
# #!/bin/bash
# # Save as ~/bin/claude or another location in your remote PATH

# lock=~/.claude/ide/$CLAUDE_CODE_SSE_PORT.lock
# if [[ -f $lock ]]; then
#     sed -ri 's/"pid":[0-9]+/"pid":'$$'/' $lock
#     /usr/local/bin/claude ${1+"$@"}  # Adjust path to actual claude binary
# else
#     echo "failed to find lock file: $lock"
#     exit 1
# fi
# #+end_example
...
#+begin_src emacs-lisp
  ...
  (defun project-dired ()
    ...
    (interactive)
    (let ((default-directory (project-root (project-current t))))
      (dired default-directory))) ; Pass default-directory to dired explicitly

  (add-to-list 'project-switch-commands '(project-shell "Shell" "s"))
  (add-to-list 'project-switch-commands '(project-dired "Dired" "d"))

  ;; Optional: If you want direct keybindings like C-x p s for shell, and C-x p d for dired
...
#+begin_src emacs-lisp
  ;; gc-cons-threshold is now set at the beginning of init
...
#+begin_src emacs-lisp
  ...
  ;; have to see why I need ensure nil here (ensure-package-ensure-guix behavior)
  (use-package drepl
    :ensure nil
    :defer t
  ...
  ;;   (add-hook 'code-cells-mode-hook #'code-cells-convert-ipynb)
    :config
    ;; ;; Configure code-cells to use jupyter for evaluation
    ;; (setq code-cells-eval-region-commands
    ;;       '((python-mode . jupyter-eval-region)))
    ...
    (with-eval-after-load 'code-cells
      (let ((map code-cells-mode-map))
        ...
        ;; Add some jupyter-specific commands
...
#+begin_src emacs-lisp
  ...
  ;; This could be simplified by just calling (make-*) and setting .authinfo with api endpoint and apikey as USER
  ...
  (with-eval-after-load 'gptel-transient
    ...
    ;; Add the suffix to the gptel-menu transient
    (transient-append-suffix 'gptel-menu
      '(0 0 0) ; This targets the suffix after the first suffix in the first group
  ...
  (use-package agent-shell
    ...
    (defun my/agent-shell-dired-marked-only ()
      ...
      (when (derived-mode-p 'dired-mode)
        (save-excursion
          (goto-char (point-min))
          ;; Only proceed if there are files explicitly marked with *.
